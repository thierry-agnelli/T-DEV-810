...
class TensorFlowManager:
    def __init__(self, model_name, epochs=5, batch_size=1, shuffle=0, repeat=None, shape=400, patience=10):
        ...
        self.batch_count = math.ceil(shuffle/batch_size)
    ...
    def save_model(self):
        if os.path.isfile(f"./models/{self.model_name}.keras"):
            os.remove(f"./models/{self.model_name}.keras")
    ...
    def train(self, train_data, validation_data):
        ...
        # loss_history = [1,2]
        # val_loss_history = [1,2]


        # Historique de l"accuracy
        accuracy_history = history.history["accuracy"]
        val_accuracy_history = history.history["val_accuracy"]
        # accuracy_history = [1,2]
        # val_accuracy_history = [1,2]
    ...
    def __cycle_augmentation(self, index, data):
        ...
        if (tf.math.floor(index/self.batch_count) + 1) % 2 == 0:
            image = tf.image.adjust_contrast(image, 0.25)
        if (tf.math.floor(index/self.batch_count) + 1) % 3 == 0:
